...
export function BookingForm({
  ...
  return (
    <section className="booking-shell" id="booking" aria-labelledby="booking-title">
      <div className="booking-shell-inner">
        ...
        <form className="booking-card" id="quote-form" noValidate onSubmit={handleSubmit(submit)}>
          ...
          <div className={`booking-row booking-row-journey${isDailyChauffeur ? " daily" : ""}`}>
            <label className={fieldClass(errors.pickup)}><span>{t("pickup", "Pick-up")}</span><div className="field-control"><Icon name="plane" className="icon" /><select id="pickup" {...register("pickup")}><option value="airport">{t("airportOption", "Antalya Airport (AYT)")}</option><option value="hotel">{t("hotelOption", "Hotel")}</option><option value="private_address">{t("privateAddressOption", "Private address")}</option></select></div><FieldErrorMessage error={errors.pickup} /></label>
            {!isDailyChauffeur && <label className={fieldClass(errors.destination)}><span>{t("destination", "Destination")}</span><div className="field-control"><Icon name="pin" className="icon" /><select id="destination" {...register("destination")}><option value="">{t("selectDestination", "Select destination")}</option>{values.pickup !== "airport" && <option value="airport">{t("airportOption", "Antalya Airport (AYT)")}</option>}{publicRouteSlugs.map((slug) => <option value={slug} key={slug}>{routeCatalog[slug].names[language as keyof typeof routeCatalog[typeof slug]["names"]] ?? routeCatalog[slug].names.en}</option>)}<option value="private_address">{t("privateAddressOption", "Private address")}</option></select></div><FieldErrorMessage error={errors.destination} /></label>}
            <label className={fieldClass(errors.vehicle)}><span>{t("vehicle", "Vehicle")}</span><div className="field-control"><Icon name="car" className="icon" /><select id="vehicle-type" {...register("vehicle")}><option value="vito" disabled={!vitoFits}>Mercedes Vito</option><option value="sprinter">Mercedes Sprinter</option></select></div><FieldErrorMessage error={errors.vehicle} /></label>
            <label className={fieldClass(errors.guests)}><span>{t("guests", "Guests")}</span><div className="field-control"><Icon name="users" className="icon" /><select id="guests" {...register("guests")}>{Array.from({ length: 13 }, (_, index) => <option value={index + 1} key={index + 1}>{index + 1}</option>)}</select></div><FieldErrorMessage error={errors.guests} /></label>
          </div>
          ...
          {isDailyChauffeur ? <>
            ...
            <div className="booking-row booking-return-row daily-departure-row">
              ...
              <label className={fieldClass(errors.travelDate)}><span>{t("arrivalDate", "Arrival date")}</span><div className="field-control"><Icon name="calendar" className="icon" /><input id="travel-date" type="date" min={minimumDate || undefined} {...register("travelDate")} /></div><FieldErrorMessage error={errors.travelDate} /></label>
              <label className={`${fieldClass(errors.arrivalTime)} time-booking-field`}><span>{t("arrivalFlightTime", "Flight arrival time")}</span><div className="field-control time-field-control" onClick={() => openTimePicker("flight-arrival-time")}><Icon name="clock" className="icon" /><span className="time-picker-value">{values.arrivalTime || t("chooseTime", "Choose time")}</span><input id="flight-arrival-time" type="time" {...register("arrivalTime")} /></div><FieldErrorMessage error={errors.arrivalTime} /></label>
              <label className={fieldClass(errors.flightNumber)}><span>{t("arrivalFlightNumber", "Arrival flight number")}</span><div className="field-control"><Icon name="plane" className="icon" /><input id="flight-number" maxLength={12} placeholder="TK1234" {...register("flightNumber")} /></div><FieldErrorMessage error={errors.flightNumber} /></label>
            </div>}
          {values.tripType === "round_trip" && <div className="booking-row booking-return-row" id="return-journey-row">
            <label className={fieldClass(errors.returnDate)}><span>{t("returnDate", "Return date")}</span><div className="field-control"><Icon name="calendar" className="icon" /><input id="return-date" type="date" min={values.travelDate || minimumDate || undefined} {...register("returnDate")} /></div><FieldErrorMessage error={errors.returnDate} /></label>
            <label className={`${fieldClass(errors.returnPickupTime)} time-booking-field`}><span>{t("returnPickupTime", "Return pick-up time")}</span><div className="field-control time-field-control" onClick={() => openTimePicker("return-pickup-time")}><Icon name="clock" className="icon" /><span className="time-picker-value">{values.returnPickupTime || t("chooseTime", "Choose time")}</span><input id="return-pickup-time" type="time" {...register("returnPickupTime")} /></div><FieldErrorMessage error={errors.returnPickupTime} /></label>
            <label className={fieldClass(errors.returnFlightNumber)}><span>{t("returnFlightNumber", "Return flight number")}</span><div className="field-control"><Icon name="plane" className="icon" /><input id="return-flight-number" maxLength={12} placeholder="TK1235" {...register("returnFlightNumber")} /></div><FieldErrorMessage error={errors.returnFlightNumber} /></label>
          </div>}
          {values.pickup === "private_address" && <div className="booking-row booking-address-row" id="pickup-address-row"><label className={fieldClass(errors.pickupAddress)}><span>{t("pickupAddress", "Full pick-up address")}</span><div className="field-control"><Icon name="pin" className="icon" /><input id="pickup-address" maxLength={160} placeholder={t("pickupAddressPlaceholder", "Hotel name, street, building number and district")} {...register("pickupAddress")} /></div><FieldErrorMessage error={errors.pickupAddress} /></label></div>}
          {!isDailyChauffeur && values.destination === "private_address" && <div className="booking-row booking-address-row" id="dropoff-address-row"><label className={fieldClass(errors.dropoffAddress)}><span>{t("dropoffAddress", "Full drop-off address")}</span><div className="field-control"><Icon name="pin" className="icon" /><input id="dropoff-address" maxLength={160} placeholder={t("dropoffAddressPlaceholder", "Hotel name, street, building number and district")} {...register("dropoffAddress")} /></div><FieldErrorMessage error={errors.dropoffAddress} /></label></div>}
          <div className="booking-row booking-options-row">
            <label className={fieldClass(errors.luggage)}><span>{t("luggageLabel", "Large luggage")}</span><div className="field-control"><Icon name="luggage" className="icon" /><select id="luggage" {...register("luggage")}>{Array.from({ length: 13 }, (_, index) => <option value={index} key={index}>{index}</option>)}</select></div><FieldErrorMessage error={errors.luggage} /></label>
          ...
          <div className="booking-footer"><p className="booking-includes">{isDailyChauffeur ? t("dailyQuoteIncludes", "Includes the selected vehicle and chauffeur with no kilometre or hour limit. Fuel is excluded.") : t("quoteIncludes", "Includes meet & greet, flight tracking, parking, waiting time and bottled water.")}</p><button className="quote-submit" type="submit" id="main-book-submit" disabled={submitting}><span>{submitting ? "…" : isDailyChauffeur ? t("reviewAndConfirm", "Review and confirm") : isPrivateAddressQuote ? t("requestQuote", "Request a price quote") : t("confirmCashBooking", "Confirm booking — pay in vehicle")}</span><Icon name="arrow-right" className="icon" /></button></div>
      ...
      {confirmation && <div className="quote-modal open" id="quote-modal" role="dialog" aria-modal="true" aria-labelledby="quote-modal-title"><button className="modal-backdrop" aria-label="Close" onClick={() => setConfirmation(null)} /><div className="modal-card"><button className="modal-close" type="button" aria-label="Close" onClick={() => setConfirmation(null)}><Icon name="close" /></button><div className="booking-confirmed"><div className="confirmed-check" aria-hidden="true"><Icon name="check" /></div><h2 id="quote-modal-title">{t("bookingConfirmed", "Booking Confirmed")}</h2><p className="confirmed-ref"><span>{t("referenceLabel", "Reference")}</span>&nbsp;<strong id="confirmed-ref">{confirmation.ref}</strong></p><p className="confirmed-msg">{confirmation.message}</p><a className="button button-gold" href={confirmation.whatsapp} target="_blank" rel="noreferrer" id="confirmed-whatsapp"><span>{t("whatsappUs", "WhatsApp us")}</span><Icon name="arrow-up-right" className="icon" /></a></div></div></div>}
    </section>
  );
}
