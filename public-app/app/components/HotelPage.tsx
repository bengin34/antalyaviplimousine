...
export function HotelPage({ hotel }: { hotel: Hotel }) {
  ...
  return <>
    ...
    <main>
      ...
      </div><aside><h2>Feste Transferpreise</h2><div className="localized-price"><p>Mercedes Vito · bis 7 Personen</p><strong>€{route.prices.vito}</strong><span>Preis für das gesamte Fahrzeug</span></div><div className="localized-price"><p>Mercedes Sprinter · bis 13 Personen</p><strong>€{route.prices.sprinter}</strong><span>Preis für das gesamte Fahrzeug</span></div></aside></div></section>
      <section className="localized-faq"><h2>Häufig gestellte Fragen</h2><article><h3>Wie lange dauert die Fahrt zum {hotel.name}?</h3><p>Bei normalem Verkehr ungefähr {route.durationLabel}.</p></article><article><h3>Was kostet der Transfer?</h3><p>Der Mercedes Vito kostet ab €{route.prices.vito} pro Fahrzeug.</p></article></section>
