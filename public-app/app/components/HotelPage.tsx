...
export function HotelPage({ hotel }: { hotel: Hotel }) {
  ...
  // Alanya is marketed as one page but priced in five sub-regions, so the
  // hotel is presented under Alanya and quoted at the price its own belde
  // carries — the same number the booking form shows.
  const priced = localizedRoute(indexedHotelBySlug(hotelSlug(hotel.name))?.region ?? hotel.regionSlug, "de") ?? route;
  ...
  return <>
    ...
    <main id="main-content" tabIndex={-1}>
      <section className="localized-route">
        ...
        <div className="localized-stats"><div><strong>{priced.durationLabel}</strong><span>Geschätzte Fahrzeit</span></div><div><strong>{priced.distance}</strong><span>Entfernung</span></div><div><strong>€{priced.prices.vito}</strong><span>Preis ab</span></div></div>
      </section>
      <section className="localized-content" id="details"><div className="localized-grid"><div>
        ...
        <p>Die Fahrt vom Flughafen Antalya nach {route.name} dauert bei normalem Verkehr ungefähr {priced.durationLabel}. Der angegebene Festpreis gilt für das gesamte Fahrzeug, nicht pro Person.</p>
      ...
      </div><div className="localized-aside"><h2>Feste Transferpreise</h2><div className="localized-price"><p>Mercedes Vito · bis 6 Personen</p><strong>€{priced.prices.vito}</strong><span>Preis für das gesamte Fahrzeug</span></div><div className="localized-price"><p>Mercedes Sprinter · bis 12 Personen</p><strong>€{priced.prices.sprinter}</strong><span>Preis für das gesamte Fahrzeug</span></div></div></div></section>
      <section className="localized-faq"><h2>Häufig gestellte Fragen</h2><article><h3>Wie lange dauert die Fahrt zum {hotel.name}?</h3><p>Bei normalem Verkehr ungefähr {priced.durationLabel}.</p></article><article><h3>Was kostet der Transfer?</h3><p>Der Mercedes Vito kostet ab €{priced.prices.vito} pro Fahrzeug.</p></article><article><h3>Was passiert bei einer Flugverspätung?</h3><p>Wir verfolgen Ihren Flug in Echtzeit und passen die Abholzeit ohne Aufpreis an.</p></article><article><h3>Wie lange wartet mein Chauffeur am Flughafen?</h3><p>Die ersten 90 Minuten nach der Landung sind kostenfrei enthalten, und bei Flugverspätungen verschiebt sich dieses Zeitfenster automatisch.</p></article><article><h3>Wie bezahle ich den Transfer?</h3><p>Bar an Ihren Chauffeur zu Beginn der Fahrt - zum Festpreis aus Ihrer Buchung, pro Fahrzeug.</p></article></section>
    ...
    <div hidden aria-hidden="true"><BookingForm selection={{ route: priced.slug, vehicle: "vito", nonce: 1 }} scrollOnSelect={false} /></div>
  </>;
}
