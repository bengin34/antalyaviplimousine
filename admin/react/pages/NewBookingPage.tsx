...
function createInitialState(): [BookingFormState, ReturnType<typeof consumeBookingPrefill>] {
  ...
  return [{
    ...
    price: '', payment: prefill?.paymentMethod ?? 'cash', status: 'confirmed', notes: prefill?.notes ?? '',
  }, prefill]
}
...
export function validateBookingForm(form: BookingFormState) {
  ...
  if (form.dropoff === 'private_address' && (dropoffAddress.length < 6 || dropoffAddress.length > 160)) return fail('Varış adresi 6-160 karakter olmalı.')
  if (form.pickup === form.dropoff && form.pickup !== 'private_address') return fail('Alış ve varış aynı olamaz.')
  if (form.pickup === 'private_address' && form.dropoff === 'private_address' && pickupAddress === dropoffAddress) return fail('Alış ve varış adresleri aynı olamaz.')
  ...
  return { error: '', payload: {
    ...
    dropoff_location: form.dropoff, dropoff_address: form.dropoff === 'private_address' ? dropoffAddress : null,
    ...
    guests, vehicle_type: form.vehicle, price_eur: isRoundTrip ? price * 2 : price,
      ...
      <form noValidate onSubmit={submit}>
        ...
        <div className="section">
          ...
          <div className="form-row">
            ...
            <Field label="Varış *"><select className="input" value={form.dropoff} onChange={e => set('dropoff', e.target.value)}>{optionList}</select></Field>
          </div>
          {(form.pickup === 'private_address' || form.dropoff === 'private_address') && <div className="form-row">
            ...
            {form.dropoff === 'private_address' && <Field label="Varış özel adresi *"><input className="input" maxLength={160} autoComplete="street-address" placeholder="Açık adresi girin" value={form.dropoffAddress} onChange={e => set('dropoffAddress', e.target.value)} required /></Field>}
          </div>}
          <div className="form-row">
            <Field label="Tarih *"><input className="input" type="date" value={form.pickupDate} onChange={e => set('pickupDate', e.target.value)} required /></Field>
            <Field label="Saat"><input className="input" type="time" value={form.pickupTime} onChange={e => set('pickupTime', e.target.value)} /></Field>
          </div>
          <div className="form-row">
        ...
        <div className="section">
          ...
          <div className="form-row">
            <Field label="Fiyat (€) *">{isRoundTrip && <span className="form-hint">Sefer başına fiyat girin</span>}<input className="input" type="number" min={0} max={999999.99} step={0.01} inputMode="decimal" value={form.price} onChange={e => set('price', e.target.value)} required /></Field>
