...
function BookingCard({ card, now, isPast, navigate, confirmPast, confirming, confirmFailed }: {
  ...
  return <div className={`card status-${card.status}${flightAlert ? ' flight-arrived' : ''}`} data-ref={card.booking_ref} data-return={String(card._isReturn)} onClick={open}>
    ...
    <div className="card-header">
      ...
      <div className="card-badges"><span className={`badge badge-${card.status}`}>{statusLabel(card.status, card.trip_type === 'round_trip')}</span>{card.trip_type === 'round_trip' && <span className={`badge ${card._isReturn ? 'badge-return' : 'badge-outbound'}`}>{card._isReturn ? 'DÖNÜŞ' : 'GİDİŞ'}</span>}</div>
    </div>
