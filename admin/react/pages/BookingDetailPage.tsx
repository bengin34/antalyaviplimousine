...
export default function BookingDetailPage({ bookingRef, isReturn, sourceTab, profitPeriod, navigate }: { bookingRef: string; isReturn: boolean; sourceTab: 'future' | 'past' | 'profit-loss'; profitPeriod?: string | null; navigate: Navigate }) {
  ...
  return <><Topbar navigate={navigate} title={dailyChauffeur ? 'Günlük Kiralama Detayı' : 'Transfer Detayı'} back={backHash} />
    <div className="scroll-area">
      ...
      <div className="section"><div className="section-label">Detaylar</div><div className="detail-grid"><div><div className="detail-key">Araç</div><div className="detail-val">{booking.vehicle_type === 'vclass' ? 'V-Class' : 'Vito'}</div></div><div><div className="detail-key">Yolcu</div><div className="detail-val">{booking.guests} kişi</div></div><div><div className="detail-key">Bagaj</div><div className="detail-val">{booking.luggage_count > 0 ? `🧳 ${booking.luggage_count}` : '—'}</div></div><div><div className="detail-key">Çocuk koltuğu</div><div className="detail-val">{booking.child_seat_count > 0 ? `👶 ${booking.child_seat_count}` : '—'}</div></div><div><div className="detail-key">Maliyet modeli</div><div className="detail-val">{booking.service_cost_mode === 'sold_transfer' && booking.sold_transfer_cost_try != null ? `${costModeLabel} · ₺${fmtPrice(booking.sold_transfer_cost_try)}` : costModeLabel}</div></div>
