...
type AdminView = 'future' | 'past' | 'budget' | 'profit-loss'
...
export function AdminTabs({ active, navigate }: { active: AdminView; navigate: Navigate }) {
  const open = (view: AdminView) => {
    ...
    else navigate(view === 'past' ? '#timeline?tab=past' : '#timeline')
  }

  const tabs: Array<[AdminView, string]> = [
    ['future', 'Gelecek'], ['past', 'Geçmiş'], ['budget', 'Bütçe'], ['profit-loss', 'Kâr/Zarar'],
  ]
