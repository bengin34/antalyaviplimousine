-- Otopark gideri 2026-09-10'da kâr hesabına girdi (20260910120000), ama
-- dağıtım defteri onu hiç tanımadı: panel toplam gidere otoparkı katarken
-- create_profit_distribution'ın mutabakat formülü katmıyordu, bu yüzden her
-- dağıtım "Profit distribution snapshot does not reconcile" ile reddediliyordu.
-- Defter artık otoparkı ayrı bir kalem olarak taşıyor; eski satırlar 0 kalır
-- (o dönemlerde otopark gideri hesaplanmıyordu), toplamları bozulmaz.

alter table public.profit_distributions
  add column if not exists parking_cost_eur numeric(14, 2) not null default 0,
  add column if not exists parking_cost_try numeric(14, 2) not null default 0;

alter table public.profit_distributions
  drop constraint if exists profit_distributions_eur_expense_total,
  drop constraint if exists profit_distributions_try_expense_total;

alter table public.profit_distributions
  add constraint profit_distributions_eur_expense_total
    check (total_expense_eur = vehicle_cost_eur + supplier_cost_eur + airport_cost_eur + parking_cost_eur + advertising_cost_eur),
  add constraint profit_distributions_try_expense_total
    check (total_expense_try = vehicle_cost_try + supplier_cost_try + airport_cost_try + parking_cost_try + advertising_cost_try);
