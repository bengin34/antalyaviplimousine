...
const files = ["index.html", "impressum.html"];
...
const uniqueFiles = [...new Set(files)].filter((file) => {
  ...
  return file === "index.html" || file === "impressum.html" || file.includes("/transfers/") || file.startsWith("transfers/") || file.includes("privacy/") || file.includes("datenschutz/") || file.includes("gizlilik/") || file.includes("impressum/") || file.includes("kunye/");
});

const records = await Promise.all(uniqueFiles.map(async (file) => {
  const url = urlFor(file);
  const isHome = ["/", "/de/", "/tr/", "/ru/"].includes(url);
  const isLegal = /privacy|datenschutz|gizlilik|impressum|kunye/.test(url);
  return { url, lastmod: await lastModified(file), changefreq: isHome ? "weekly" : isLegal ? "yearly" : "monthly", priority: isHome ? "1.0" : isLegal ? "0.2" : "0.8" };
}));
