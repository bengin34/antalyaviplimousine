...
const pages = {
  en: {
    ...
    bodies: [
      ...
      "Google Analytics and Google Ads are loaded on every visit in a consent-limited mode: until you accept in the privacy dialog no cookies are set, no advertising identifiers are sent and the transmitted data cannot be linked to you. If you accept, cookies are set and usage, device, interaction and conversion data may be sent to Google. The provider is Google Ireland Limited and processing by affiliated companies outside the European Economic Area may occur. Independently of Google, we record anonymous usage events in our own database (page, booking step, route and price); they contain no name, e-mail address, telephone number or IP address and cannot be linked to you. You may withdraw consent at any time through Privacy settings. Rejecting analytics does not affect booking functions.",
  ...
  de: {
    ...
    bodies: [
      ...
      "Google Analytics und Google Ads werden bei jedem Besuch in einem eingeschränkten Modus geladen: Solange Sie im Datenschutzdialog nicht einwilligen, werden keine Cookies gesetzt, keine Werbe-Identifikatoren übermittelt und die übertragenen Daten lassen sich Ihnen nicht zuordnen. Mit Ihrer Einwilligung werden Cookies gesetzt und Nutzungs-, Geräte-, Interaktions- und Conversion-Daten können an Google Ireland Limited übermittelt werden. Eine Verarbeitung außerhalb des Europäischen Wirtschaftsraums kann stattfinden. Unabhängig von Google erfassen wir anonyme Nutzungsereignisse in unserer eigenen Datenbank (Seite, Buchungsschritt, Strecke und Preis); sie enthalten weder Namen, E-Mail-Adresse, Telefonnummer noch IP-Adresse und lassen sich Ihnen nicht zuordnen. Sie können Ihre Einwilligung jederzeit über die Datenschutzeinstellungen widerrufen. Eine Ablehnung beeinträchtigt die Buchung nicht.",
  ...
  tr: {
    ...
    bodies: [
      ...
      "Google Analytics ve Google Ads her ziyarette sınırlı bir modda yüklenir: gizlilik penceresinden onay vermediğiniz sürece çerez yerleştirilmez, reklam tanımlayıcısı gönderilmez ve iletilen veriler sizinle ilişkilendirilemez. Onay verirseniz çerezler yerleştirilir; kullanım, cihaz, etkileşim ve dönüşüm verileri Google Ireland Limited şirketine iletilebilir ve Avrupa Ekonomik Alanı dışında işlenebilir. Google'dan bağımsız olarak kendi veritabanımızda anonim kullanım olayları (sayfa, rezervasyon adımı, rota ve fiyat) kaydediyoruz; bunlar ad, e-posta, telefon veya IP adresi içermez ve sizinle ilişkilendirilemez. Onayınızı Gizlilik ayarlarından istediğiniz zaman geri çekebilirsiniz. Reddetmek rezervasyon işlevlerini etkilemez.",
  ...
  ru: {
    ...
    bodies: [
      ...
      "Google Analytics и Google Ads загружаются при каждом посещении в ограниченном режиме: пока вы не дали согласие в окне конфиденциальности, файлы cookie не устанавливаются, рекламные идентификаторы не передаются, а передаваемые данные нельзя связать с вами. После согласия устанавливаются файлы cookie, а данные об использовании, устройстве, взаимодействиях и конверсиях могут передаваться Google Ireland Limited и обрабатываться за пределами Европейской экономической зоны. Независимо от Google мы записываем анонимные события использования в собственную базу данных (страница, шаг бронирования, маршрут и цена); они не содержат имени, адреса электронной почты, номера телефона или IP-адреса и не могут быть связаны с вами. Согласие можно отозвать через настройки конфиденциальности. Отказ не влияет на бронирование.",
