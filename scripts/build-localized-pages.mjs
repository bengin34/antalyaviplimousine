...
<main><section class="localized-route"><div class="eyebrow light"><span></span><p>Antalya VIP Tourism</p></div><h1>${text.h1(name)}</h1><p>${text.routeDescription(name, vitoPrice)}</p><div class="localized-stats"><div><strong>${duration}</strong><span>${text.duration}</span></div><div><strong>${localized.distance}</strong><span>${text.distance}</span></div><div><strong>€${vitoPrice}</strong><span>${text.from}</span></div></div></section>
