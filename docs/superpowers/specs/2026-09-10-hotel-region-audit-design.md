...
- `src/hotel-distances.js` carries a `district` field that looks like
  ...
  (`scripts/lib/hotel-distances-merge.mjs:35-36`) writes `district:
...
`npm run generate:hotel-index-discovered`, so a hand edit is silently reverted
on the next run. The correction belongs in whichever source artifact holds that
Place ID — `mergeReviewedHotelMatches` concatenates two of them, so the skill
looks the Place ID up in both `region-price-matches.json` and
`region-review-resolved.json` rather than assuming the first — followed by a
regeneration. Layer 2 therefore has two distinct edit procedures, and the report
must say which population a row belongs to so the right one is used.

### Prerequisite: the generator is stale, and running it is not neutral

`src/hotel-index-discovered.js` does not currently match what its own generator
would produce. `mergeReviewedHotelMatches` merges `region-price-matches.json`
(351 rows) with `region-review-resolved.json` (22 rows) for 373, but the
checked-in file holds only the 351, and none of the 22 resolved Place IDs appear
in it. Its header still reads "Review rows are intentionally excluded", which
the generator no longer honours. Both files were last written by the same commit
(`260a7211`): the merge was added and the generator was never re-run.

This turns the discovery correction path into a trap. Regenerating as a side
effect of fixing one hotel's region would add 22 hotels to `hotelIndex`
(1223 → 1245). None of them has a `hotelDistances` entry, so
`src/hotel-distances.test.js`'s "every indexed hotel has a numeric km" test goes
red — layer 2's `npm test` step fails for a reason unrelated to the correction,
and 22 fresh unaudited rows appear exactly when layer 3 is trying to drive the
unchecked count to zero.

The plan must settle this **before** the correction loop is usable, as its own
first task. Two options:

- **Adopt the 22.** Regenerate, run `build-hotel-distances.mjs` for the new
  slugs, and audit them like any other row. This honours the merge that was
  deliberately written, and folds the 22 into the same verification everything
  else gets.
- **Keep them excluded.** Restore the filter the header comment describes, so
  the generated file matches its stated contract and regeneration becomes a
  no-op.

This is a commercial decision — the 22 are bookable hotels the index would start
offering — so it is the operator's call, not the plan's. Whichever is chosen,
regeneration must be proven a no-op before any discovery-row correction relies
on it.

If the 22 are adopted, one claim in the Problem section softens: those rows came
through `applyReviewedPlaceOverride`, a manual override path, rather than
`selectOperationalHotelPlace`, so their Place IDs are not evidence of the same
strength and they should not be assumed identity-verified going in.
...
as well as the generator change. Order matters: the strip pass runs *after*
`applyResult` stops writing the key, or the next `build-hotel-distances` run
re-introduces it for every row it touches. The inert `district` fixtures in
